@extends('layouts.master') @section('title','Edit Order')
@section('css')
<style type="text/css">
        ...
        <div class="panel panel-bordered">
            ...
            <div class="panel-body">
                <form role="form" class="form-horizontal form-validation" method="post" enctype="multipart/form-data">
                    ...
                    <h2>
                        <a href="#" id="addScnt">
                            <button type="submit" class="btn btn-primary" id="add"><i class="fa fa-shopping-cart"></i>  Add New Product</button>	           
                        </a>
                    </h2>
                    <table  class="table dynatable table-bordered bordered table-striped table-condensed datatable" >
                        ...
                        <tbody id="p_scents" >
                            ...
                            <tr>
                                <td>
                                    <select name="product[]" style=" width: 100%;" id="product"  onchange="getunitPrice" >
                                        <option value="">Select Product</option>
                                        @for ($x =0; $x < sizeof($product); $x++) 
                                        <option @if ($orderList[$i]->products == $product[$x]->id) selected @endif @if ($orderList[$i]->products != $product[$x]->id) disabled @endif  value="{{$product[$x]->id}}">{{$product[$x]->product_Name}}</option>    
                                        @endfor
                                    </select>
                                </td>           
                                <td><input type="text" name="unitprice[]" value="{{$orderList[$i]->unitPrice}}" id="unitprice{{$orderList[$i]->id}}"/></td>
                                <td><input type="text" name="quantity[]" id="quantity{{$orderList[$i]->id}}" value="{{$orderList[$i]->itemCount}}" onchange="getTotalCurentPrice({{$orderList[$i]->id}})"/></td>
                                <td><input type="text" name="totalPrice[]" value="{{ $orderList[$i]->unitPrice * $orderList[$i]->itemCount}}" id="totalPrice{{$orderList[$i]->id}}"/></td>
                                <td><a href="#" class="red type-delete" onclick="deleteoderItem({{$orderList[$i]->id}})"><i class="fa fa-trash-o"></i></a></td>
